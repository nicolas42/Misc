...
#define KEY_REPEAT_INTERVAL 0.00f // Interval between repeats in seconds
...
int main(int argc, char *argv[]) {
    ...
    while (!WindowShouldClose()) {
        ...
        if ((IsKeyDown(KEY_LEFT_SUPER) || IsKeyDown(KEY_RIGHT_SUPER)) && IsKeyPressed(KEY_EQUAL)) {
            zoomScale += 0.1;
            zoomScale = Clamp(zoomScale, 0.1f, 5.0f); // Limit zoom
        }
        if ((IsKeyDown(KEY_LEFT_SUPER) || IsKeyDown(KEY_RIGHT_SUPER)) && IsKeyPressed(KEY_MINUS)) {
            zoomScale -= 0.1;
            zoomScale = Clamp(zoomScale, 0.1f, 5.0f); // Limit zoom
        }
